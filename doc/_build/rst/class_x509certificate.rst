...
They can be used as the server certificate in StreamPeerSSL.accept_stream (along with the proper :ref:`CryptoKey<class_CryptoKey>`), and to specify the only certificate that should be accepted when connecting to an SSL server via StreamPeerSSL.connect_to_stream.
